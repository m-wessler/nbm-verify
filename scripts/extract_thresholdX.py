...
# We need to make this flexible by CWA using shapefiles
# For now we can just use fixed bounds
# cwa = 'SEW'
nlat, xlat = 30, 50#46.0, 49.0
nlon, xlon = -130, -100#-125.0, -120.5
...
def unpack_fhr(nbm_file):
    ...
    if os.path.isfile(nbm_file):
        ...
        try:
            ...
            return data.isel(x=slice(idx[1].min(), idx[1].max()), 
...
if __name__ == '__main__':
    ...
    for forecast_hour in np.arange(24, 180+1, 6):
        ...
        if not os.path.isfile(outdir+outfile):
            ...
            with mp.get_context('fork').Pool(96) as p:
